...
sub vcl_backend_response {
    ...
    if (beresp.ttl > 0s && (bereq.method == "GET" || bereq.method == "HEAD")) {
        ...
        # Strip "Cookie" from the Vary header before storing. hash_data() above already keys on
        # the one cookie that matters (X-Magento-Vary); leaving "Cookie" in Vary makes Varnish's
        # own vary-matching key the object off the raw Cookie header, which never repeats between
        # requests once you factor in ever-changing cookies like requestId/section_data_ids -
        # guaranteeing a MISS on every real browser request.
        if (beresp.http.Vary) {
            set beresp.http.Vary = regsuball(beresp.http.Vary, "\s*,?\s*Cookie\s*,?\s*", ",");
