...
function mysqlQuery()
{
    CMD="${BIN_MYSQL} -h${DB_HOST} -u${DB_USER} -P${DB_PORT} --password=\"${DB_PASSWORD}\" --execute=\"${SQLQUERY}\"";
    runCommand
}
...
function restore_db()
{
    ...
    if [[ "${_DB_FILENAME}" =~ "db_mydumper.sql.gz" ]]; then
        ...
        CMD="gunzip -cf ${_DB_FILENAME} | myloader --host ${DB_HOST} --user ${DB_USER} --password ${DB_PASSWORD} --database ${DB_NAME} --port ${DB_PORT} --overwrite-tables --verbose 1 --stream"
    else
